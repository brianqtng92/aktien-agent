...
[CONFLICT] (NEU, v11.22, 2026-09-17, ANET-Feedback-Runde) — reine AEGIS-SYNTHESE-EBENE, kein Tag, den JJ/Conan sich selbst vergeben (jede KI sieht nur die eigene Recherche): wird von Aegis vergeben, wenn JJ und Conan für DIESELBE Kennzahl auf erkennbar unterschiedlichen Datenbasen/Quellen stehen (nicht nur unterschiedliche Gewichtung, sondern unterschiedliche zugrunde liegende Zahlen). Wird AUSSCHLIESSLICH in der Prosa-Synthese und der Dissensus-Map (Agent-Playbook.md Punkt 52) verwendet, NIEMALS als Inline-Tag in der DNA-Check-Tabelle (Brians ausdrückliche Entscheidung 2026-09-17, Status quo seit der Tabellen-Verschlankung vom 08.09. bleibt bestehen).
...
→ Moat-Komponenten-Zerlegung des obigen 0-4-Gesamtscores: siehe KSF-SCORECARD unten, Regel 53 — läuft jetzt dort INNERHALB des Moat-/Wettbewerbsposition-Faktors, kein separates System mehr (Korrektur v11.23, Conans Redundanz-Fund).
...
🧩 MOAT-KOMPONENTEN-ZERLEGUNG BEIM MOAT-/WETTBEWERBSPOSITIONS-FAKTOR (NEU v11.22, integriert v11.23, 2026-09-17, Regel 53, ANET-Feedback-Runde): genau der obige KSF-Faktor, der die Moat-/Wettbewerbsposition betrifft, wird NICHT nur mit einem einzelnen ✅/🟡/❌ abgehandelt, sondern in 3-5 branchenspezifisch benannte Unter-Komponenten aufgefächert (typisch: Technologie/Produkt, Software/Ökosystem, Switching-Costs/Kundenbindung, Skalen-/Netzwerkvorteile, Marke/Reputation — nur die einschlägigen, nicht mechanisch alle durchziehen), je mit eigener Stärke-Einschätzung UND separater Trend-Angabe (verbessernd/stabil/leicht abnehmend/zusammenbrechend) — ein Unternehmen kann in EINER Komponente elite und in einer anderen unter Druck sein, das verschwindet sonst in einem einzelnen Status. **Bleibt Teil der KSF-Scorecard, kein zweites paralleles Scoring-System** (Korrektur v11.23 — die erste Fassung führte dies noch als eigenständigen Block unter MOAT-VERIFIKATION, Conans Gegenprüfung fand die Gefahr eines Doppelsystems). Kein Zusatzaufwand: nutzt dieselbe Moat-/Wettbewerbsrecherche.
...
→ KUNDEN-CAPEX-VERKNÜPFUNG (NEU, v11.22, 2026-09-17, Regel 6-Erweiterung, ANET-Feedback-Runde): bei benannter Top-1-Konzentration >15-20% zusätzlich die eigene, öffentlich kommunizierte Capex-/Investitionsguidance DIESES Kunden recherchieren (dessen eigener Earnings-Call/10-K, nicht der des analysierten Unternehmens) und explizit gegen die eigene Wachstumsthese stellen (z.B. "Kunde X guidet Capex +40% 2026, stützt Umsatzthese" vs. "Kunde X guidet Verlangsamung, Risiko für Wachstumsannahme"). Entfällt ersatzlos bei geringerer Konzentration.
...
Kapitalintensiver Ramp-up (≥2 von 3 Triggern, Agent-Playbook Punkt 55) → Szenario-Ertragskraftmodell ERSETZT FULL DCF, siehe SCHRITT 5E — NUR wenn TMR-würdiger Umsatz UND laufender transformativer Kapitalausbau gleichzeitig vorliegen (Normalfall bleibt FULL DCF)
...
⚠ ZIRKULARITÄTS-SPERRE (NEU, v11.20, 2026-09-17, Regel 38b): „näher am aktuellen Marktpreis" oder „plausibler" ALLEIN ist KEINE zulässige Begründung für die TV-Methoden-Wahl — das verschiebt die Optimismus-Annahme nur vom Wachstum (Gordon-g) in den Terminal-Multiple, statt sie zu rechtfertigen. Die Pflicht-Begründung MUSS explizit adressieren, WARUM das Unternehmen im Terminal-Jahr (J5) noch das angenommene Peer-Median-Multiple verdient — Wettbewerbsposition zu diesem Zeitpunkt, Margen-Nachhaltigkeit, erwartete Wachstumsverlangsamung Richtung Branchen-Norm. Bei einer offenen strukturellen Moat-Kernfrage (z.B. ein direkter Großkonkurrent mit eigenem Ökosystem-Vorteil) ist das Exit-Multiple NICHT automatisch die plausiblere Methode, nur weil es höher ausfällt — eine ungelöste Moat-Frage kann genau GEGEN ein hohes Terminal-Multiple sprechen, nicht nur dafür.
⚠ EXIT-MULTIPLE-SENSITIVITÄT — 3×3-MATRIX (NEU v11.21, ausgebaut v11.22, 2026-09-17, Regel 38c, ANET-Feedback-Runden): statt einer 1D-Linie eine echte 2D-Matrix — 3 Umsatz-CAGR-Stufen (Bear/Base/Bull-Wachstumsannahme) × 3 Exit-Multiple-Stufen (Peer-Median −15% / Peer-Median / Peer-Median +15%), analog zur bestehenden 3×3-WACC×Wachstum-Matrix (Punkt 24, Reverse-DCF-Seite). Zeigt die volle Wechselwirkung beider Annahmen statt nur eine Achse zu variieren — am aussagekräftigsten bei einer offenen strukturellen Frage, wo Wachstum UND Multiple beide von derselben Unsicherheit abhängen. Kein Zusatzaufwand: nutzt dieselben Peer-Daten wie S3b. Entfällt ersatzlos, wenn S3b selbst entfällt (<3 Peers).

📐 KNOWN / UNKNOWN / UNKNOWABLE (NEU, v11.22, 2026-09-17, ChatGPT-Vorschlag, ANET-Feedback-Runde): direkt neben der Sensitivitäts-Matrix eine knappe 3-Felder-Liste — KNOWN (hart belegte Ist-Zahlen: Margen/Umsatz/Kundenkonzentration/Cash/ROIC), UNKNOWN (grundsätzlich recherchierbar, aber noch offen: z.B. zukünftiger Marktanteil eines Konkurrenten), UNKNOWABLE (langfristig nicht auflösbar: Nachfrage 2030+, künftiges Terminal-Multiple). Verhindert, dass eine breite DCF-Spanne als Rechenungenauigkeit missverstanden wird statt als Ausdruck echter Unsicherheit.
→ **"Die 3 nächsten Infos" (NEU, v11.24, 2026-09-18, ChatGPT-Vorschlag, MP-Materials-Vergleich):** aus der UNKNOWN-Zeile die 3 Punkte herauspicken, deren Klärung die Unsicherheit am stärksten reduzieren würde — nicht einfach alle UNKNOWN-Punkte auflisten, sondern priorisiert nach Entscheidungsrelevanz. Reine Kuratierung der bereits vorhandenen UNKNOWN-Liste, keine neue Recherche.
...
SCHRITT 5E — 🏗️ RAMP-UP-SZENARIOMODELL (NEU, v11.24, 2026-09-18, Regel 55, Raketentonis MP-Materials-Report via ChatGPT-Gegenprüfung des ANET-Reports)
⚠ NUR wenn mindestens 2 von 3 Triggern gleichzeitig vorliegen — sonst entfällt dieser Schritt ersatzlos, S1–S5 (FULL DCF) bleibt der Normalfall:
① Laufendes Einzelprojekt/Capex-Programm mit Investitionssumme, die einen wesentlichen Teil der Marktkapitalisierung ausmacht oder laut eigener Guidance die Kapazität/das Geschäft transformiert (nicht normale Erhaltungs-Capex)
② Vom Unternehmen selbst kommunizierte Ziel-Wirtschaftlichkeit (Marge/EBITDA/FCF-Guidance) liegt ≥2 Jahre in der Zukunft, nicht aus der Ist-Lage ableitbar
③ Aktuelles FCF negativ/knapp positiv DURCH den Ausbau selbst (nicht durch operative Schwäche des Kerngeschäfts)
→ Trigger-Check als Pflicht-Einzeiler ausweisen: „Ramp-up-Trigger: ①[Ja/Nein] ②[Ja/Nein] ③[Ja/Nein] → [ausgelöst/nicht ausgelöst]"

Bei Auslösung ERSETZT dieses Modell S1–S5 vollständig (kein paralleles FULL DCF, keine Doppelarbeit):
R1: Ziel-Adj.-EBITDA im dokumentierten Zieljahr je Szenario — AUS Unternehmens-Guidance/Kapazitätsplan, NICHT erfunden (Bear = konservatives Ende der Guidance oder eigene Verzögerungs-Annahme, Base = Guidance-Mittelwert, Bull = oberes Guidance-Ende bei fristgerechter Umsetzung)
R2: Exit-Multiple-Band je Szenario — peer-abgeleitet (dieselben Peer-EV/EBITDA-Werte wie S3b/Punkt 19), mit 1-Satz-Begründung je Szenario, warum das Multiple zu diesem Zeitpunkt/Reifegrad passt
R3: Nettofinanzposition im Zieljahr je Szenario (Cash+kurzfristige Anlagen − Schulden, unter Berücksichtigung des bis dahin verbrauchten/finanzierten Capex)
R4: Aktienzahl = ÖKONOMISCH VOLL VERWÄSSERTE Aktienzahl aus dem Verwässerungs-Wasserfall (Regel 43), NICHT die Basis-Aktienzahl — je Szenario ggf. unterschiedlich (Bull: weniger neue Verwässerung bei früher Selbstfinanzierung; Bear: mehr durch zusätzlichen Finanzierungsbedarf)
R5: Diskontsatz-BAND statt Einzel-WACC (z.B. 9-12%, aus WACC-Breakdown oben abgeleitet, aber als Spanne statt Punktschätzung)
→ Barwert/Aktie je Szenario = (R1 × R2 + R3) diskontiert mit R5, geteilt durch R4

Pflicht-Kennzeichnung im Output: „Kein präziser DCF, sondern ein transparenter Belastungstest für ein kapitalintensives Ramp-up-Unternehmen" — verhindert Scheingenauigkeit bei einem Modell mit inhärent hoher struktureller Unsicherheit. Ersetzt in diesem Fall auch den generischen Stress-Test-Block unten (Bear/Base/Bull sind hier bereits die drei Szenarien oben, keine zweite Bear/Base/Bull-Rechnung nötig). TV-Cross-Check (S3b) und Known/Unknown/Unknowable oben entfallen bei Auslösung ersatzlos (Gordon-Growth-TV ist hier nicht das Modell) — die PROJEKT-ÖKONOMIE-KETTE (Capex-Programm → Zeitplan/Baufortschritt → Zielkapazität → Qualifizierungs-/Hochlaufstatus → Auslastungspfad → Ziel-EBITDA/FCF) wird stattdessen als Kontext-Spalte in die ohnehin bestehende Meilenstein-Timeline (Aegis-Synthese-Ebene, Agent-Playbook Punkt 44) übernommen — kein separater Recherche-Schritt hier.

Stress-Test (entfällt bei ausgelöstem Ramp-up-Szenariomodell, siehe oben):
...
-----
### 🧠 SCHRITT 7B — THESE-MONITORING-KETTE (NEU v11.22, umgebaut v11.23, 2026-09-17, Regel 51, ANET-Feedback-Runde — Pflicht bei FULL DEEP DIVE, optional bei QUICK FILTER)

**Ersetzt die frühere getrennte Führung von "Kern-Annahmen" und "Kipppunkten" (v11.22) — Conans Gegenprüfung fand beide plus Teile von Kill-Sheet/KPI-Liste redundant, jetzt EINE verkettete Tabelle statt vier Listen:**

4-6 Kernannahmen benennen, auf denen das eigene Rating TATSÄCHLICH beruht (nicht generische Floskeln), je Zeile mit vier Spalten:

| Kernannahme | Confidence + Beleg | Beobachtbarer KPI | Eigener Kipppunkt |
|---|---|---|---|
| [z.B. "EOS bleibt echter Software-Moat trotz NVIDIA"] | [Hoch/Mittel/Niedrig — 1 Satz Beleg] | [konkrete Kennzahl, an der sich das prüfen lässt] | [was würde MEINE Einschätzung genau BEI DIESER Annahme anheben/senken] |

→ Wo der "Beobachtbare KPI" eine kontinuierliche Kennzahl ist UND ein Kill-Sheet-Trigger direkt dieselbe Annahme betrifft (Regel 26): die graduierte Eskalations-Schwelle (🟡/🟠/🔴) hier eintragen statt sie zusätzlich im Kill-Sheet zu wiederholen — Kill-Sheet behält nur seine EIGENEN, nicht-annahmengebundenen Ereignis-Trigger (Litigation, Insider-Verkaufsserien u.ä.).
→ Diese Tabelle wird aus der bereits durchgeführten Analyse extrahiert, kein separater Rechercheschritt. Speist zusätzlich die Top-10-KPI-Liste (Aegis-Synthese, Agent-Playbook.md Punkt 44) — deren Kandidaten kommen bevorzugt aus der Spalte "Beobachtbarer KPI" hier, keine unabhängig davon kuratierte zweite Auswahl.

**Abhängigkeits-Check (NEU, v11.24, 2026-09-18, Regel 56, ChatGPT-Vorschlag, MP-Materials-Vergleich):** direkt unter der Tabelle EIN Satz dazu, ob eine der 4-6 Kernannahmen gleich MEHRERE der anderen Zeilen trägt (z.B. bei MP Materials: scheitert "10X läuft 2028 fristgerecht an", fallen Magnetkapazität, Kundenumsätze, EBITDA UND Bewertung gleichzeitig) — oder ob die These stattdessen breiter diversifiziert ist (mehrere voneinander unabhängige Stützen, keine einzelne davon trägt allein). Keine neue Tabelle, keine neue Recherche — reine Einordnung der bereits vorhandenen Kernannahmen-Kette nach Abhängigkeitstiefe statt nur nach Einzel-Confidence. Format: „Abhängigkeitstiefe: [konzentriert auf Annahme X / breit diversifiziert] — [1 Satz Begründung]."
...
38. TV-CROSS-CHECK-PFLICHT (NEU, v11.14, 2026-09-15, nach Abgleich mit Anthropics `/dcf`-Skill aus `anthropics/financial-services`): Sofern ≥3 vergleichbare Peers mit EV/EBITDA vorliegen (siehe Peer-Multiple-Vergleichstabelle, Agent-Playbook.md Punkt 19), wird die Gordon-Growth-Terminal-Value (S3) zusätzlich über eine Exit-Multiple-Terminal-Value (S3b, Peer-Median-EV/EBITDA × EBITDA_J5) gegengeprüft — kein separater Rechercheschritt, nutzt bereits vorhandene Peer-Daten. Abweichung >30% zwischen beiden TV-Methoden → Pflicht-Kommentar, welche Methode hier plausibler ist. Bei <3 Peers entfällt S3b ersatzlos, kein künstlicher Peer-Satz. Ändert NICHT die primäre Base-Case-FV (bleibt Gordon-Growth-basiert) — reine Zusatz-Absicherung, analog zur bestehenden Reverse-DCF-Sanity-Rolle (Regel 11). **Regel 38b — ZIRKULARITÄTS-SPERRE (NEU, v11.20, 2026-09-17, ausgelöst durch Gemini-Gegenprüfung des ANET-Reports):** die Pflicht-Begründung bei >30%-Abweichung darf sich NICHT auf "näher am Marktpreis"/"plausibler" allein stützen — muss explizit begründen, warum das Peer-Median-Exit-Multiple im Terminal-Jahr noch verdient wird (Wettbewerbsposition/Margen-Nachhaltigkeit/Wachstumsnormalisierung), sonst wird die Optimismus-Annahme nur vom Wachstum in den Terminal-Multiple verschoben statt gelöst. **Regel 38c — EXIT-MULTIPLE-SENSITIVITÄT (NEU v11.21, ausgebaut v11.22, 2026-09-17):** zusätzlich zur qualitativen Begründung (38b) wird die Exit-Multiple-FV als echte 3×3-Matrix (3 Umsatz-CAGR-Stufen × 3 Peer-Multiple-Stufen −15%/Basis/+15%) gerechnet, um die volle Wechselwirkung beider Annahmen sichtbar zu machen, statt nur eine Achse zu variieren.
