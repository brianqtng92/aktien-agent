...
Makro-Analyse (Fear&Greed, Indizes, Zinsen, FX, Rohstoffe) läuft NICHT MEHR bei Jack selbst — das lief zuvor redundant bei allen 3 KIs einzeln (Token-Verschwendung ohne Zusatzwert). Läuft jetzt zentral bei Aegis (Jarvis) und wird bei Bedarf ins Fact-Pack an Jack mitgeliefert.
...
→ Nicht mitgeliefert → Phasenbestimmung weiterhin ausschließlich aus Eigenhistorie, mit explizitem Vermerk „Leitindikator nicht mitgeliefert – Phasenurteil basiert nur auf Eigenhistorie". Kein eigener Web-Search-Aufwand bei Jack dafür.
