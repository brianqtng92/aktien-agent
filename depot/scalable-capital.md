...
## Update 2026-09-24 (Bank Central Asia komplett verkauft — jetzt live bestätigt)

Brian hatte den Komplett-Verkauf von **Bank Central Asia** gemeldet (Teil einer größeren, 6 Positionen umfassenden Verkaufswelle, siehe `depot/finanzen-net-zero.md` für die übrigen 5). Die Scalable-MCP-Anbindung war zunächst nicht verbunden (wiederkehrender Auth-Ausfall) und wurde von Brian im Lauf der Session wiederhergestellt — **per Live-Abruf (`list_portfolio_transactions`, `get_portfolio_holdings`, `get_portfolio_cash_breakdown`) jetzt bestätigt:**

**SELL-Transaktion, 24.09.2026 ~18:30 UTC: 6.199,871861 Anteile @ Gesamterlös 1.809,37 €.** Investsumme 1.999,94 € → **realisiert -190,57 € (-9,5%)**, korrigiert die zuvor mit dem letzten bekannten Kurs (1.921,96 €, 18.09.) angesetzte Näherung (-77,98 €/-3,9% war zu optimistisch). Erlös liegt seither als Cash auf dem Scalable-Verrechnungskonto (`get_portfolio_cash_breakdown`: Cash-Balance/Kaufkraft 1.809,37 € — exakt der Verkaufserlös, noch nicht reinvestiert oder abgezogen).
